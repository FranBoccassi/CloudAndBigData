...
csvDF = spark.read.option("header","true").csv(path)
csvDF2 = spark.read.option("header","true").csv(path2)
...
countedPairs = pairs.groupby("Tag").count().orderBy("Count",ascending = 0).show()
